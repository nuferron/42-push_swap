...
//la merda aquesta de funcio te mes de 25 linies a sobre de no funcionar es llarga

int	sorting_five(t_stack **a, t_stack **b, t_stack *last, char *prev)
{
	if (!(*b) && ((*a)->id == 0 || (*a)->id == 4) && *prev != 'b')
	{
		*prev = 'a';
		push(a, b);
		return (write(1, "pb\n", 3));
	}
	if (b && is_sorted(*a) == 0 && *prev != 'a')
	{
		*prev = 'b';
		push(b, a);
		return (write(1, "pa\n", 3));
	...
	if (*prev != 'r' && (consec((*a)->id, (*a)->next->id) == 1
		|| consec((*a)->id, last->id) == 1))
	{
		*prev = 'R';
		reverse_rotate(a);
		return (write(1, "rra\n", 4));
	...
	{
		ft_printf("no conditions matched\n");
		return (-1);
	}
}
